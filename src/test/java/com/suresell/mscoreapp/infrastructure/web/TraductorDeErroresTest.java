...
class TraductorDeErroresTest {
    ...
    @Test
    @DisplayName("🔴 el cuerpo trae el código bajo `codigo`, que es la clave que lee el panel")
    void elCuerpoTraeElCodigoQueLeeElPanel() {
        // El panel rellena `motivo.codigo` desde `codigo` o `code` y de ninguna
        // otra (`core/ui/motivo-del-error.ts:52`). Este servicio emitía solo
        // `error`, así que el panel nunca reconocía el código y caía en su
        // respaldo por estado HTTP. Si alguien quita `codigo`, el panel deja de
        // distinguir SIN_PRECIO de cualquier otro 422, y en silencio.
        ResponseEntity<Map<String, Object>> r = traductor.reglaDeNegocio(
                new com.suresell.mscoreapp.shared.exception.ReglaDeNegocioException(
                        "SIN_PRECIO", "price", "«Gaseosa» no tiene precio."));

        assertThat(r.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
        assertThat(r.getBody()).containsEntry("codigo", "SIN_PRECIO");
        assertThat(r.getBody()).containsEntry("error", "SIN_PRECIO");
        assertThat(r.getBody()).containsEntry("campo", "price");
    }

    @Test
    @DisplayName("la clave `codigo` viaja también en los errores de la base, no solo en las reglas")
    void elCodigoViajaTambienDesdeLaBase() {
        ...
        assertThat(r.getBody()).containsEntry("codigo", "NO_EXISTE");
    }
